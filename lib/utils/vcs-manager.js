...
export default class VCS{
  ...
  static initPathes(){
    ...
    fs.makeTreeSync( atom.getConfigDirPath() + "/temp" );
    VCS.zipPath = atom.getConfigDirPath() + "/temp/tmp.zip";
    VCS.tempPath = atom.getConfigDirPath() + "/temp/temp" + (new Date()).getTime();
    VCS.packagePath = VCS.packagesPath + "/medicus-engine-helper";
  }
  ...
  static async installEngineHelperPacakage(options){
    ...
    try{
      ...
      await VCS.deleteFolder(VCS.tempPath);
  ...
  static async copyPackageToPackagesFolder(){
    ...
    let fromPath = fs.readdirSync(VCS.tempPath).map( file => VCS.tempPath+ "/" + file)[0];
